...
public class LANBroadcastService : MonoBehaviour {
  ...
  private string playerIP; // Players ip adress
  ...
  void Update() {
    ...
    if((currentState == enuState.Searching || currentState == enuState.Announcing)
      ...
      objUDPClient.Send(objByteMessageToSend, objByteMessageToSend.Length, new IPEndPoint(IPAddress.Broadcast, 22403));
      // Restart the timer
  ...
  private void EndAsyncReceive(IAsyncResult objResult) {
    ...
    if(objByteMessage.Length > 0 &&
        !objSendersIPEndPoint.Address.ToString().Equals(playerIP)) {
  ...
  private void StartBroadcastingSession() {
    ...
    if(currentState != enuState.NotActive)
      StopBroadCasting();
    // Create the client
    objUDPClient = new UdpClient(22403);
